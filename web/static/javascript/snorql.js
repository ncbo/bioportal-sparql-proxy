...
function Snorql() {
    ...
    this.start = function() {
        ...
        if (queryString.match(/csrfmiddlewaretoken=/)) {
        ...
        var querytext = null;
        if (browse && browse[1] == 'example1') {
            var resultTitle = 'ontologies with their name and acronym';
            var query = 'SELECT ?ont ?name ?acr\n' +
                    'WHERE { \n?ont a omv:Ontology .\n?ont omv:acronym ?acr .\n?ont omv:name ?name . \n}\n';
        }
        if (browse && browse[1] == 'example2') {
            var resultTitle = 'ontologies sorted by number of classes';
            var query = 'SELECT ?ont ?name ?nclasses\n' +
                    'WHERE { \n?ont a omv:Ontology .\n?ont omv:name ?name .\n?ont omv:numberOfClasses ?nclasses .\n}\n'+
			'ORDER BY DESC(xsd:integer(?nclasses))\n';
        }
	...
	if (browse && browse[1] == "example3") {
            var resultTitle = 'number of ontologies per category/domain';
var query = 'SELECT ?category ?name (count(?ontology) as ?counter) WHERE {'+
 '\n   ?category a omv:OntologyDomain .'+
 '\n   ?category omv:name ?name .'+
 '\n   ?ontology omv:hasDomain ?category .'+
'\n} GROUP BY ?category';
	}
	...
	if (browse)
        ...
        service.query(query,token, {
            success: successFunc,
            failure: function(report) {
                if (report.status == 400) {
                    try {
                        error_line = report.responseText.match(/on line ([^&]*)/)[1];
                    } catch (e) {
                        error_line = null;
                    }
                    if (error_line) {
    ...
    this._displayEndpointURL = function() {
        /*
        var newTitle = 'Snorql: Exploring BioPortal at ' + this._endpoint;
    ...
    this.submitQuery = function() {
        ...
        if (mode == 'browse') {
            ...
            document.getElementById('query').value = getEditor().getValue();
        } else {
...
function SPARQLResultFormatter(json, namespaces) {
    ...
    this._getLinkMaker = function(varName) {
        if (varName == 'property') {
            return function(uri) { return '?property=' + encodeURIComponent(uri) + "&csrfmiddlewaretoken=" + csrf_global; };
        } else if (varName == 'class') {
            return function(uri) { return '?class=' + encodeURIComponent(uri) + "&csrfmiddlewaretoken=" + csrf_global; };
        } else {
            return function(uri) { return '?describe=' + encodeURIComponent(uri) + "&csrfmiddlewaretoken=" + csrf_global; };
        }
    }
